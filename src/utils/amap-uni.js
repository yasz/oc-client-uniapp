...
AMapWX.prototype.getGeo = function (a) {
    var b = this.requestConfig,
        c = a.options
    b = { key: this.key, extensions: 'all', s: b.s, platform: b.platform, appname: this.key, sdkversion: b.sdkversion, logversion: b.logversion }
...
AMapWX.prototype.getRegeo = function (a) {
    function b(d) {
        ...
        wx.request({
            ...
            data: { key: c.key, location: d, extensions: 'all', s: e.s, platform: e.platform, appname: c.key, sdkversion: e.sdkversion, logversion: e.logversion },
            method: 'GET',
            header: { 'content-type': 'application/json' },
            success: function (g) {
                if (g.data.status && '1' == g.data.status) {
                    ...
                    f = f.join('')
                    a.success([{ iconPath: a.iconPath, width: a.iconWidth, height: a.iconHeight, name: f, desc: k, longitude: m, latitude: n, id: 0, regeocodeData: g }])
...
AMapWX.prototype.getWeather = function (a) {
    function b(g) {
        ...
        wx.request({
            ...
            data: { key: d.key, city: g, extensions: h, s: e.s, platform: e.platform, appname: d.key, sdkversion: e.sdkversion, logversion: e.logversion },
            method: 'GET',
            header: { 'content-type': 'application/json' },
            success: function (f) {
    ...
    function c(g) {
        wx.request({
            url: 'https://restapi.amap.com/v3/geocode/regeo',
            data: { key: d.key, location: g, extensions: 'all', s: e.s, platform: e.platform, appname: d.key, sdkversion: e.sdkversion, logversion: e.logversion },
            method: 'GET',
            header: { 'content-type': 'application/json' },
            success: function (h) {
...
AMapWX.prototype.getPoiAround = function (a) {
    function b(e) {
        e = { key: c.key, location: e, s: d.s, platform: d.platform, appname: c.key, sdkversion: d.sdkversion, logversion: d.logversion }
                ...
                if (g.data.status && '1' == g.data.status) {
                    if ((g = g.data) && g.pois) {
                        for (var h = [], f = 0; f < g.pois.length; f++) {
                            ...
                            h.push({ latitude: parseFloat(g.pois[f].location.split(',')[1]), longitude: parseFloat(g.pois[f].location.split(',')[0]), iconPath: k, width: 22, height: 32, id: f, name: g.pois[f].name, address: g.pois[f].address })
                        }
...
AMapWX.prototype.getTransitRoute = function (a) {
    ...
    wx.request({
        ...
        header: { 'content-type': 'application/json' },
        success: function (c) {
            c && c.data && c.data.route && ((c = c.data.route), a.success({ distance: c.distance || '', taxi_cost: c.taxi_cost || '', transits: c.transits }))
        },
        fail: function (c) {
            a.fail({ errCode: '0', errMsg: c.errMsg || '' })
        },
