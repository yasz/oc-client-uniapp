<template>
  <view>
    ...
    <view class="flex baseline" v-for="(file, index) in localFileList" :key="file.srcname">
      <view class="pr-10 pb-6" @click="open(file)">
        {{
          file.srcname.length > 20
            ? `${file.srcname.substr(0, 12)}..${file.srcname.match(/\.\w+$/)[0]
            }`
            : file.srcname
        }}
      </view>
      <u-image @click="deleteFile(index)" src="/src/static/delete.png" width="20rpx" height="20rpx" />
    </view>

    <view v-if="localFileList?.length < maxCount">

      <u-upload accept="all" @afterRead="afterRead" @oversize="oversize" multiple :maxCount="maxCount"
        :maxSize="5000000">
        <view class="px-4">
...
const afterRead = async (event: any) => {
  ...
  for (const file of files) {
    try {
      ...
      let res = await uploadFile({ file, url: "attachments:create?attachmentField=assignment_submission.attachment" });
      newFiles.push({
        id: res.data.id,
